...
global.assertLoadTest = async (
  ...
  if (result.requestsPerSecond < minRequestsPerSecond) {
    failures.push(`Requests/sec ${result.requestsPerSecond.toFixed(2)} below minimum ${minRequestsPerSecond}`);
  }

  if (result.avgResponseTime > maxAvgResponseTime) {
    failures.push(`Avg response time ${result.avgResponseTime.toFixed(2)}ms exceeds maximum ${maxAvgResponseTime}ms`);
  }
...
global.monitorMemory = (name: string, fn: () => Promise<any> | any) => {
  ...
  const runFn = async () => {
    ...
    console.log(
      `  Diff: RSS=${afterMemory.rss - beforeMemory.rss}MB, Heap=${afterMemory.heapUsed - beforeMemory.heapUsed}MB`,
    );
